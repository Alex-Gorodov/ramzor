...
export function AddMissionForm(): JSX.Element {
  ...
  const handleCompleteSoldiers = useMemo(() => {
    return completeMission(formData.participants, formData.numOfCommanders, users).missionGroup;
  ...
  const handleFormSubmit = (evt: FormEvent<HTMLFormElement>) => {
    ...
    dispatch(addMission({
      mission: {
        ...
        name: formData.name,
        length: Math.round(formData.lengthHours) + formData.lengthMinutes / 60,
        isSpecial: formData.isSpecial,
        startingDate: new Date(`${formData.startingDate}T${formData.startHours}:${formData.startMinutes ? formData.startMinutes : '00'}:00.000Z`),
        startingTime: Math.round(formData.startHours) + formData.startMinutes / 60,
        numOfCommanders: Number(formData.numOfCommanders),
    ...
    <div className="form__container" style={{
      ...
      <div className="form__wrapper">
        <div className="form__inner-wrapper">
          ...
          <button className='form__close-btn' onClick={() => {dispatch(toggleForm({isOpened: !isFormOpened}))}}></button>
        </div>
        ...
        <form action="" method="post" className="mission-form form" ref={formRef} onSubmit={handleFormSubmit}>
            ...
            <ul className="form__radio-group form__list-group">
              <li>
                <label className="form__element" htmlFor="mission-regular">
                  <input type="radio" name="isSpecial" id="mission-regular" onChange={handleChangeData}/>
                  <span>בת״ש</span>
                </label>
              </li>
              <li>
                <label className="form__element" htmlFor="mission-special">
                  <input type="radio" name="isSpecial" id="mission-special" onChange={handleChangeData}/>
                  <span>מיוחדת</span>
                </label>
              </li>
            </ul>
          ...
          <fieldset className="form__fieldset form__fieldset--column">
            <label className="form__element grid-form-element" htmlFor="mission-start">
              <span>התחלה:</span>
              <div>
                <input className="form__input form__input--date" type="date" onChange={handleChangeData} name="startingDate" id="mission-starting-date" style={{marginLeft: '32px'}}/>

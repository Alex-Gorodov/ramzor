...
export function ScheduledMission({mission}: ScheduledMissionProps): JSX.Element {
  ...
  return (mission &&
    <div className="mission" style={{
      top: `calc(${mission.startingTime / 24 * 100}%)`,
      ...
      height: `calc(${mission.length * 42 + mission.length}px)`
    }}>
      ...
      <div className="mission__info">
        <span>
        {
            ...
            ?
            `0${Math.round(mission.startingTime) + Math.round(mission.length)}`
            :
            `${Math.round(mission.startingTime) + Math.round(mission.length)}`
          }:{
        ...
        <div className="mission__description" style={{bottom: showInfo ? '30px' : '-500px'}}>
          {mission.description}
        </div>
      </div>
    </div>
